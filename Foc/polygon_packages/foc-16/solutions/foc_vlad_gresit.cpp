    ...
    for (int i = 1; i <= n; ++i)
        for (int j = 1; j <= m; ++j)
            if(!viz[i][j] && A[i][j]) {
                ...
                if (min_x == 1 || max_x == n || min_y == 1 || min_y == m)
                    continue;
                else {
                    ++nr_comp;
                    ...
                    ++prec_xy[min_x - 1][min_y - 1];
                    --prec_xy[min_x - 1][max_y + 2];
                    --prec_xy[max_x + 2][min_y - 1];
                    ++prec_xy[max_x + 2][max_y + 2];
                }
            }
    for (int i = 1; i <= n; ++i)
        for (int j = 1; j <= m; ++j) 
            prec_xy[i][j] += prec_xy[i - 1][j] + prec_xy[i][j - 1] - prec_xy[i - 1][j - 1]; 
